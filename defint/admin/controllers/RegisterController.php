...
class RegisterController{
 ...
 {
    $register_query="INSERT INTO admin (fName,lName,otherName,email,nia,regimentalNo,number,password,gender,region,subDivision,rank) VALUES('$fName','$lName','$otherName','$email','$nia','$regimentalNo','$number','$password','$gender','$region','$subDivision','$rank')";
...
{
$checkRegimentalNo="SELECT regimentalNo FROM admin WHERE regimentalNo='$regimentalNo' LIMIT 1";
...
{
    $checkUserNumber="SELECT number FROM admin WHERE number='$number' LIMIT 1";
...
{
    $checkUserEmail="SELECT email FROM admin WHERE email='$email' LIMIT 1";
...
{
  $checkUserNia="SELECT nia FROM admin WHERE nia='$nia' LIMIT 1";
...
//Checking if NIA Exist
public function isNiaExist($nia)
{ 
  $checkManager="SELECT nia FROM admin WHERE nia ='$nia' LIMIT 1";
